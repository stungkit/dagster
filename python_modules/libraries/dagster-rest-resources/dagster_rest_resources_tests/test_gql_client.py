...
class TestDagsterPlusGraphQLClientGetData:
    def test_returns_parsed_data(self):
...
class TestDagsterPlusGraphQLClientExecuteGeneric:
    def test_returns_parsed_result(self):
        client = _make_dagster_plus_client()
        response_data = {"test_k": "test_v"}
        ...
        result = client.execute_generic("{ test }")

        assert result == response_data


class TestDagsterPlusGraphQLClientOverridesUsedByGeneratedMethods:
    def test_returns_parsed_result(self):
        settings = {"sso_default_role": "VIEWER"}
        response_data = {"organizationSettings": {"settings": settings}}
        ...
        client = _make_dagster_plus_client()
        client.execute = Mock()
        client.execute.return_value = _make_mock_response(response_data)
    ...
    def test_raises_unauthorized_error(self):
        response_data = {
            "organizationSettings": {
                ...
                "message": "test not allowed",
            }
        }

        client = _make_dagster_plus_client()
        client.http_client = create_autospec(httpx.Client, spec_set=True, instance=True)
        client.http_client.post.return_value = _make_mock_response(response_data)
    ...
    def test_raises_graphql_error(self):
        ...
        with pytest.raises(DagsterPlusGraphqlError, match="test error message"):
            client.get_organization_settings()
