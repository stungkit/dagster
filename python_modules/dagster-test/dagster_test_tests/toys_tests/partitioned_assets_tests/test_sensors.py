    ...
    with instance_for_test() as instance:
        with build_sensor_context(
            ...
            for ints_sensor in ints_sensors:
                ...
                assert len(result.run_requests) == 1  # pyright: ignore[reportOptionalMemberAccess,reportArgumentType,reportAttributeAccessIssue]  # ty: ignore[invalid-argument-type,unresolved-attribute]
                assert len(result.dynamic_partitions_requests) == 3  # pyright: ignore[reportOptionalMemberAccess,reportArgumentType,reportAttributeAccessIssue]  # ty: ignore[invalid-argument-type,unresolved-attribute]
...
def test_daily_partitioned_sensor():
    with build_sensor_context(
        ...
        assert len(result.run_requests) == 2  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
