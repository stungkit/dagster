...
def _normalize_value(value: Any, key: str) -> str:
    ...
    if not error and not seven.json.loads(serialized_value) == value:  # pyright: ignore[reportPossiblyUnboundVariable]  # ty: ignore[possibly-unresolved-reference]
        error = f'JSON encoding "{serialized_value}" of value "{value}" is not equivalent to original value'  # pyright: ignore[reportPossiblyUnboundVariable]  # ty: ignore[possibly-unresolved-reference]
    if error:
        raise DagsterInvalidDefinitionError(
    ...
    return serialized_value  # pyright: ignore[reportPossiblyUnboundVariable]  # ty: ignore[possibly-unresolved-reference]
