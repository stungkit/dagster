...
def test_metadata_asset_materialization():
    ...
    materialization = materialization_events[0].event_specific_data.materialization  # pyright: ignore[reportAttributeAccessIssue,reportOptionalMemberAccess]
...
def test_metadata_asset_observation():
    ...
    observation = observation_events[0].event_specific_data.asset_observation  # pyright: ignore[reportAttributeAccessIssue,reportOptionalMemberAccess]
...
def test_parse_invalid_metadata():
    ...
    with pytest.raises(DagsterInvalidMetadata) as _exc_info:
        normalize_metadata(metadata)  # ty: ignore[invalid-argument-type]

    normalized = normalize_metadata(metadata, allow_invalid=True)  # ty: ignore[invalid-argument-type]
...
def test_table_column_keys():
    with pytest.raises(TypeError):
        dg.TableColumn(bad_key="foo", description="bar", type="string")  # pyright: ignore[reportCallIssue]  # ty: ignore[unknown-argument, missing-argument]
...
def test_table_column_values(key, value):
    ...
    with pytest.raises(CheckError):
        dg.TableColumn(**kwargs)  # ty: ignore[invalid-argument-type]


def test_table_constraints_keys():
    with pytest.raises(TypeError):
        dg.TableColumn(bad_key="foo")  # pyright: ignore[reportCallIssue]  # ty: ignore[unknown-argument, missing-argument]
...
def test_table_column_constraints_keys():
    with pytest.raises(TypeError):
        dg.TableColumnConstraints(bad_key="foo")  # pyright: ignore[reportCallIssue]  # ty: ignore[unknown-argument]
...
def test_table_schema_keys():
    with pytest.raises(TypeError):
        dg.TableSchema(bad_key="foo")  # pyright: ignore[reportCallIssue]  # ty: ignore[unknown-argument, missing-argument]
