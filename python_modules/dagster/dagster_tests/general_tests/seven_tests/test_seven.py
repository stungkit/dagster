...
def test_is_fn_or_decor_inst():
    ...
    def quux_decor(fn):
        q = Quux()
        return update_wrapper(q, fn)  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
...
def test_is_subclass():
    ...
    assert not is_subclass(2, dg.DagsterType)  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
