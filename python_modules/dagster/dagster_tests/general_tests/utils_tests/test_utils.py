...
def test_event_generation_manager():
    ...
    with pytest.raises(CheckError, match="Not a generator"):
        EventGenerationManager(None, int)  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]

    with pytest.raises(CheckError, match="must be a class"):
        EventGenerationManager(basic_generator(), None)  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
