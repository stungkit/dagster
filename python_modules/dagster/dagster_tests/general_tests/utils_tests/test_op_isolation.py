...
def test_execute_nested_graphs():
    ...
    res = nested_graph.execute_in_process()  # pyright: ignore[reportAttributeAccessIssue]  # ty: ignore[unresolved-attribute]
...
def test_single_op_with_bad_inputs():
    ...
    assert failure_data.error.cls_name == "DagsterTypeCheckDidNotPass"  # pyright: ignore[reportOptionalMemberAccess]  # ty: ignore[unresolved-attribute]
    assert (
        'Type check failed for step input "num_two" - expected type "Int"'
        in failure_data.error.message  # pyright: ignore[reportOptionalMemberAccess]  # ty: ignore[unresolved-attribute]
    )
