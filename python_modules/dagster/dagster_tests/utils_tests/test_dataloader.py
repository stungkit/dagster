...
class ThingLoader(DataLoader[str, Thing]):
    def __init__(self):
        super().__init__(batch_load_fn=batch_load_fn)  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
...
def test_bad_load_fn():
    ...
    async def _test():
        loader = DataLoader(_oops)  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
        ...
        with pytest.raises(TypeError):
            done[0].result()  # pyright: ignore[reportIndexIssue]  # ty: ignore[invalid-key]
