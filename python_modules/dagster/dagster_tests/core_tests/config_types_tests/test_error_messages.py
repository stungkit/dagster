...
def test_invalid_optional_in_config():
    with pytest.raises(
        ...
        @dg.op(config_schema=dg.Optional[int])  # ty: ignore[invalid-argument-type]
        def _op(_):
            pass
...
def test_invalid_dict_call():
    ...
    with pytest.raises(TypeError, match=re.escape("'DagsterDictApi' object is not callable")):
        ...
        @dg.op(config_schema=dg.Dict({"foo": int}))  # pyright: ignore[reportCallIssue]  # ty: ignore[call-non-callable]
        def _op(_):
            pass
...
def test_non_scalar_key_map():
    with pytest.raises(
        ...
        @dg.op(config_schema={dg.Noneable(int): str})  # ty: ignore[invalid-argument-type]
        def _op(_):
            pass
