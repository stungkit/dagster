...
def test_filter_runs_no_retry_runs_on_step_failures(instance_no_retry_runs_on_step_failures):
    instance = instance_no_retry_runs_on_step_failures
    ...
    assert any(
        "Not retrying run since it failed due to a step failure." in str(event)
